...
public class FireRuleAndOrSeq {
    ...
    public static void main(String[] args) {
        ...
        String siddhiApp = "" +
                ...
                "" +
                "@info(name = 'query1') " +
                "from every (h = humidityStream [percentage < 25] or t = temperatureStream [value > 45]) within 5 seconds " +
                "select t.value, h.percentage, h.area, t.timestamp " +
        ...
        for (int i = 0; i < Integer.parseInt(args[0]); i++) {
            new Thread(() -> {
                ...
                while (keepRunning) {
                    try {
                        hsInputHandlers[id].send(new Object[]{22, "office", System.nanoTime()});
                        //rInputHandler.send(new Object[]{System.currentTimeMillis()});
                        tsInputHandlers[id].send(new Object[]{46, "office", System.nanoTime()});
