...
public class TcpServerSampleThreeStateRule {
    static final Logger LOG = Logger.getLogger(TcpServerSampleThreeStateRule.class);
        ...
        for (int i = 0; i < Integer.parseInt(args[1]); i++) {
            new Thread(() -> {
                ...
                String inStreamDefinition = "" +
                        ...
                        "" +
                        " define stream fireStream (value int, percentage int, density int, area string);";
                String query = "" +
                        " @info(name = 'query1') " +
                        " from every (h = humidityStream [percentage < 25] -> t = temperatureStream [value > 45] -> s = smokeStream [density > 50]) within 5 seconds " +
                        " select value, percentage, density, h.area " +
