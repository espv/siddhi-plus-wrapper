    ...
    protected void processAndClear(ComplexEventChunk<StreamEvent> streamEventChunk) {
        ...
        synchronized (patternSyncObject) {
            while (streamEventChunk.hasNext()) {
                ...
                stabilizeStates();  // Significant amount of time is spent processing here
                currentStreamEventChunk.add(streamEvent);
                ComplexEventChunk<StateEvent> eventChunk = ((StreamPreStateProcessor) next).
                        processAndReturn(currentStreamEventChunk);  // Actual event processing occurs here, and a significant portion of time is spent processing here.
        ...
        while (retEventChunk.hasNext()) {
            ...
            querySelector.process(new ComplexEventChunk<>(stateEvent, stateEvent, false));  // 8 - A lot of processing is spent in here.
        }
    }
